...
class Stresstracking
{
    function getWeeklyAvg($today){
        ...
        $stmt = Database::getDb()->prepare("SELECT ROUND(AVG(t.stress_factor),2) AS 'Average' FROM User AS u INNER JOIN Task AS t ON u.UID = t.created_by WHERE u.UID = :UID AND t.end_date >= DATE_SUB(:today, INTERVAL 7 DAY) AND t.end_date <= :today");
